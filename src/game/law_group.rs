    ...
    pub fn new(inp: ObjectReader<Utf8Encoding>) -> Result<Self, Box<dyn Error>> {
        ...
        for (key, _, value) in inp.fields() {
            match key.read_str().as_ref() {
                "progressive_movement_chance" => {
                    ret.progressive_movement = value.read_scalar()?.to_f64()?
                }
                "regressive_movement_chance" => {
                    ret.regressive_movement = value.read_scalar()?.to_f64()?
                }
                "law_group_category" => ret.law_group_category = value.read_string()?.to_uppercase(),
                "enactment_approval_mult" => {
                    ret.enactment_approval_mult = value.read_scalar()?.to_f64()?
                }
