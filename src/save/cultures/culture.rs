...
impl Culture {
    pub fn new(inp: ObjectReader<Utf8Encoding>) -> Result<Self, Box<dyn Error>> {
        ...
        for (key, _, value) in inp.fields() {
            match key.read_str().as_ref() {
                ...
                "obsessions" => {},
                "potential_migration" => {},
                "core_states" => homelands = value.read_array()?.values().map(|x| x.read_string()).try_collect()?,
                a => println!("\t\t\t\t\"{a}\" => {{}},")
            }
        }
        Ok(Self {
    ...
    pub fn new_group(inp: ObjectReader<Utf8Encoding>) -> Result<HashMap<usize, Option<Self>>, Box<dyn Error>> {
        ...
        for (key, _, value) in inp.fields() {
            ret.insert(key.read_str().parse()?, value.read_object().ok().map(|x| Culture::new(x)).transpose()?);
        }
        Ok(ret)
    }
}
