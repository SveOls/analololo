...
impl Pop {
    pub fn new(inp: ObjectReader<Utf8Encoding>) -> Result<Self, Box<dyn Error>> {
        ...
        for (key, _, value) in inp.fields() {
            match key.read_str().as_ref() {
                ...
                "weekly_budget" => weekly_budget = Some(value.read_array()?.values().map(|x| x.read_scalar().map(|x| (x.to_f64().unwrap() * 10e4) as i64)).try_collect()?),
                "partial_growth_wa" => growth_workforce = (value.read_scalar()?.to_f64()? * 10e4) as i64,
                "partial_growth_dn" => growth_dependents = (value.read_scalar()?.to_f64()? * 10e4) as i64,
    ...
    pub fn new_group(inp: ObjectReader<Utf8Encoding>) -> Result<HashMap<usize, Option<Self>>, Box<dyn Error>> {
        ...
        for (key, _, value) in inp.fields() {
            ret.insert(key.read_str().parse()?, value.read_object().ok().map(|x| Pop::new(x)).transpose()?);
        }
        Ok(ret)
    }
}
